...
-- HOSTILE SEARCH - by Rvotri - Code Information ...	

-- Not sure if it was correct place to add THIS file, but let's go
-- My part of code will optionally add random set hostiles units around downed pilot
-- to make rescue more Challenging, you can set your options in code around lines 130 ~180.
-- I am not a developer, so my code was all about i learned practicing ...
-- ORIGINAL CODE WAS NOT MODIFIED ...

-- CODE DESCRIPTION                 Around Lines...
-- Options, Vars, SIDE NOTES        [ 150 ~ 190 ]
-- Main Function                    [ 260 ~ 480 ]
-- Trigger Call for Function        [ 670 ~ 680 ]
-- Aditional Messaging 4 LZ Hot	    [ 1370 ~ 1380 ]
...
-- ----------------------------------------------------------------------------------------------
-- HOSTILE SEARCH FOR DOWNED PILOT
-- ----------------------------------------------------------------------------------------------

-- Spawns some hostiles around downed pilot that Will offer some fire against SAR teams ...

csar.hostilesOnDowned = true   		--  Enable or Not (true or false) - See "Side Notes" down below to prevent downed pilot to be killed by hostiles in sight ... 
-- Unit Types Quantityes below ...			
csar.hostilesSoldiersNumMin = 5			-- 	Controls Min number of hostiles Soldiers that will possible spawns (0 means eventually none will be spawned) ...
csar.hostilesSoldiersNumMax = 8			-- (0 = off) Controls max number of hostiles Soldiers that will possibly spawns ...
csar.hostilesSoldiersLMG = 2			-- (0 = off) Number of soldiers with Light Machine Guns ...
csar.hostilesSoldiersRPG = 1			-- (0 = off) Number of soldiers with RPG-7 ...
csar.hostilesManpads = 0				-- (0 = off) Number of Soldiers with MANPADS (This turns Rescue a lot harder for light Helos) ...
csar.hostilesTruck = 2					-- (0 = off, Strongly recommended to On) Qtty of transport truck providing ammo for hostile group ...	
csar.hostilesLightVehArmed = 1			-- (0 = off) Qtty of light armed vehicle in the hostile group providing a nice manner to SAR teams to get shot :´(
-- Unit Spawn Dispersion and Distance to Downed pilot ....
csar.hostilesOffsetMin = -450			-- Minimum distance (in Meters?) in Offset for X and Y that Hostiles will spawn from downedpilot ...
csar.hostilesOffsetMax = 450			-- Maximum distance (in Meters?) in Offset for X and Y that Hostiles will spawn from downedpilot ...

-- TODO csar.hostilesEnableROEonClose = true	-- Prevent Hostiles from keep shooting in Downned until SAR get around 3kms, prevents Lags, and save bullets for real action


-- Sample Templates for...		Smin 	SMax	LMGs	RPGs	MNPDS	Trucks	LgtArmd
	-- Light Helicopters 	>	5,		9, 		2 		1, 		0, 		2, 		1 
	-- Light Attack Helos 	>	3, 		5, 		2, 		2,		1, 		3, 		3
	-- Attack Helos 		>	0, 		3, 		4		1		2		3,		4 


-- >> SIDE NOTES << --
 
	-- REMEMBER to set  "csar.imortalcrew = true" OR "csar.invisiblecrew = true" down below or downed pilot will be killed by hostiles in sight ...
	-- IF "csar.hostilesTruck" is set to 0, i recommend to set "csar.invisiblecrew" as true down bellow, otherwise enemy will have no ammo for SAR teams ...
	-- ALSO For every 4 units in Hostile group, i recommend to set 1 truck in "csar.hostilesTruck", otherwise, several units will get no ammo very quickly ...
	-- HOSTILE code will respect values in original "csar.enableForAI, csar.enableForRED and csar.enableForBLUE" vars ...
	-- Mind your mission size. Setting to spawn lots units will not only affect the Spawn moment, but in a mission where lots players eject will spawn a lot of groups with several units
	
	-- 	If you use Only Positive values or only negative values for both hostilesOffsetMin and hostilesOffsetMax, the Hostile group will be spawned grouped
	--	like they're coming from one direction, if you Mix  negative and positive values they will spawn all around Downed pilot, like surrounding it from all directions ...
	-- 	ALSO, setting only positive or negatives, Offset will work like Minimal and Max distance, but mixing it, enemies may spawn very close to Downed pilot 
	-- 	(Ex: between -100 and 100 there is 1 (1 meter from Downed ))
	

	-- TODO >
	-- Get a way to detect name of close Hostile group to set ROE > Weapons Free (csar.hostilesEnableROEonClose)
	-- /TODO
...
-- -----------------------------------------------------------------------------------------------------
-- ------------- HOSTILE SEARCH SPAWNING FUNCTION CODE --------------------------------------------------------
-- -----------------------------------------------------------------------------------------------------

function hostilesSpawn (downedPilot)

	local downedSide = downedPilot:getCoalition()	-- Holds coalition as integer for downed pilot (1 Russia / 2 USA / 3 Gaia(?) )...
	local downedPos = downedPilot:getPoint()		-- Holds position from Downed pilot ...
	local offsetX = 100								-- Holds Integer that will be randomized to generate random Offset from downed pilot in X Axis ...
	local offsetY = 100								-- Holds Integer that will be randomized to generate random Offset from downed pilot in Y Axis ...
	local heading = 180								-- Holds Integer that will be randomized to generate random Heading for Units in group ...
	local tmpInteger =	1							-- Holds Integer that will be randomized to generate random vehicle in LAVs constructor ...
	local j = 1										-- Holds Integer That will be the Index Counter number to each unit in the group ...
	local i = 1										-- Holds Integer That will be the Counter number to quantity of unit type in the group ...
	
	-- Construct Table for bad guys ...
	local _idBadGuy = mist.getNextGroupId()
	local _groupNameBadGuy = "BadGuy-" .. _idBadGuy
	local _groupBadGuy = {
		["visible"] = true,
		["groupId"] = _idBadGuy,
		["hidden"] = false,
		["units"] = {},
		["name"] = _groupNameBadGuy,
		["task"] = {},
	}
	-- Checks side (1 Russia / 2 USA / 3 Gaia(?) )
	if downedSide == 2 then
	
	-- Generate Random quantity and construct INFANTRIES ...
		local numHostiles = math.random (csar.hostilesSoldiersNumMin, csar.hostilesSoldiersNumMax)
		
			
		while( j <= numHostiles )
		do
			offsetX = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			offsetY = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			-- if downedSide == 2 then -- Checks side to add proper soldier to group ... (I think this scripting method is Heavier ...)
			_groupBadGuy.units[j] = csar.createUnit(downedPos.x + offsetX, downedPos.z + offsetY, 10, "Infantry AK")
			--else
			--	_groupBadGuy.units[j] = csar.createUnit(downedPos.x + offsetX, downedPos.z + offsetY, 10, "Soldier M4")
			j = j+1
		end
			
		-- Checks and construct TRUCKS in group ...
		i = 1
		while ( i <= csar.hostilesTruck )
		do
			offsetX = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			offsetY = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			heading = math.random(0,359)
			_groupBadGuy.units[j] = csar.createUnit(downedPos.x + offsetX, downedPos.z + offsetY, heading, "KAMAZ Truck")
				j = j+1
				i = i+1
		end
			
		-- Checks and constructs LIGHT ARMED VEHICLES ...
		i = 1
		while ( i <= csar.hostilesLightVehArmed )
		do 
			offsetX = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			offsetY = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			heading = math.random(0,359)
			tmpInteger = math.random(1,4) -- Gen Random Number to spawn Random light Armored truck (USE 1,5 Here to enable BRDM-2)
			
			local armouredName = "tt_KORD" -- Just in case :3
			
			if tmpInteger == 1 then
				armouredName = "tt_KORD"	
			elseif tmpInteger == 2 then
				armouredName = "tt_DSHK"
			elseif tmpInteger == 3 then
				armouredName = "HL_KORD"
			elseif tmpInteger == 4 then
				armouredName = "HL_DSHK" 
			elseif tmpInteger == 5 then	-- "BRDM-2" Is a Scout option, BUT it's damn Hard to beat with light helos (To enable use 1,5 in X on line "tmpInteger = math.random(1,X)"
				armouredName = "BRDM-2" 
			end 
				
			_groupBadGuy.units[j] = csar.createUnit(downedPos.x + offsetX, downedPos.z + offsetY, heading, armouredName)
			j = j+1
			i = i+1
		end 
			
			
	-- Checks and add LMG(s) to group ... 
		i = 1
		while ( i <= csar.hostilesSoldiersLMG )
		do 
			offsetX = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			offsetY = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			heading = math.random(0,359)
			_groupBadGuy.units[j] = csar.createUnit(downedPos.x + offsetX, downedPos.z + offsetY, heading, "Paratrooper AKS-74")
			j = j+1
			i = i+1
		end 
		
	-- Checks and adds RPG-7(s) to the group ... 
		i = 1
		while ( i <= csar.hostilesSoldiersRPG )
		do 
			offsetX = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			offsetY = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			heading = math.random(0,359)
			_groupBadGuy.units[j] = csar.createUnit(downedPos.x + offsetX, downedPos.z + offsetY, heading, "Paratrooper RPG-16")
			j = j+1
			i = i+1
		end	
		
	-- Checks and add Manpad(s) to Group ...
		i = 1
		while ( i <= csar.hostilesManpads )
		do 
			offsetX = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			offsetY = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			heading = math.random(0,359)
			_groupBadGuy.units[j] = csar.createUnit(downedPos.x + offsetX, downedPos.z + offsetY, heading, "SA-18 Igla manpad")
			j = j+1
			i = i+1
		end	
		
	-- Spawns RED hostile Group ...
		coalition.addGroup(country.id.RUSSIA, Group.Category.GROUND, _groupBadGuy)
		...
		do
			...
			offsetY = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			heading = math.random(0,359)
			_groupBadGuy.units[j] = csar.createUnit(downedPos.x + offsetX, downedPos.z + offsetY, heading, "M 818")
			j = j+1
			i = i+1
		end
		...
		do 
			...
			offsetY = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			heading = math.random(0,359)
			tmpInteger = math.random(1,5) -- Gen Random Number to spawn Random light Armored truck 
			...
			elseif tmpInteger == 4 then
				armouredName = "HL_DSHK" 
			elseif tmpInteger == 5 then
			armouredName = "M1043 HMMWV Armament" 
			end 
	...
	-- Checks and add LMG(s) to group ... 
		...
		do 
			...
			offsetY = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			heading = math.random(0,359)
			_groupBadGuy.units[j] = csar.createUnit(downedPos.x + offsetX, downedPos.z + offsetY, heading, "Soldier M249")
			j = j+1
			i = i+1
		end 
	...
	-- Checks and add Manpad(s) to Group ...
		...
		do 
			...
			offsetY = math.random(csar.hostilesOffsetMin,csar.hostilesOffsetMax)
			heading = math.random(0,359)
			_groupBadGuy.units[j] = csar.createUnit(downedPos.x + offsetX, downedPos.z + offsetY, heading, "Soldier stinger")
			j = j+1
			i = i+1
		end	
		
		-- Spawns BLUE hostile Group ...
		coalition.addGroup(country.id.USA, Group.Category.GROUND, _groupBadGuy)
...
-- -HOSTILES SEARCH CODE ------------------------------------------------------------------------------------------------
		
		-- Checks, IF hostiles on Downed  active then calls for Hostiles code ...
			if csar.hostilesOnDowned == true then
				hostilesSpawn(_unit)
			end
...
function csar.checkCloseWoundedGroup(_distance, _heliUnit, _heliName, _woundedGroup, _woundedGroupName)
    ...
    if csar.heliVisibleMessage[_lookupKeyHeli] == nil then
		...
		-- HOSTILES SEARCH CODE ----------------------------------------------------------------------------------------------
		-- Part of hostilesOnDowned code as extra messaging and Hostiles ROE[TODO] set On ... --------------------------------
		if csar.hostilesOnDowned == true then
			--if hostilesEnableROEonClose == true then
				-- TODO -  SET ROE ON AI HOSTILES
			--end 
			csar.displayMessageToSAR(_heliUnit, string.format("%s: %s. LZ is hot!! I'm under Heavy enemy fire, Take care!!!", _heliName, _pilotName), 30)
		else
